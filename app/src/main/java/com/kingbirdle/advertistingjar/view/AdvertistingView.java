...
import com.kingbirdle.advertistingjar.application.MyApplication;
...
import static com.kingbirdle.advertistingjar.utils.SpUtil.readString;
...
//        initView(context);
    ...
    public void initAdvertising(Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= M) {
            if (SpUtil.readBoolean(Const.PERMISSONS)) {
                startAdvertisting();
                Plog.e("已获得相关权限");
            } else {
                Plog.e("开始申请权限");
    ...
    public void startAdvertisting() {
        ...
        init(getActivity());
        String cpuId = SpUtil.readString(Const.CPU_ID);
        if (cpuId.isEmpty()) {
            cpuId = MacUtil.getCpuId(activity);
            SpUtil.writeString(Const.CPU_ID, cpuId);
        } else {
...
//        playInitialize();
    ...
    public void setAppPackageName(String packageName) {
        SpUtil.writeString(Const.MAIN_APP_NAME, packageName);
    }
//
    ...
    private class MyHandler extends Handler {
        ...
        private void handleMessage2(Message msg, Parameter parameter, String id) {
            switch (msg.what) {
                ...
                case 4:
                    ...
                    e("播放类型：" + playType);
                    if (playType == 1) {
                        String controlNumber = readString(Const.CONTROL_NUMBER);
                        String ad = "AD";
        ...
        private void controlPlayAnswer(final String id, final String rCode, final byte[] command, final byte[] number, final int playType, final int isResult) {
            ThreadManager.getInstance().doExecute(new Runnable() {
                @Override
                public void run() {
                    String controlNumber = readString(Const.CONTROL_TYPE);
                    switch (controlNumber) {
                        case "LAN":
                            e("配置软件控制回应");
    ...
    private void handleMessage3(Message msg, final String id) {
        switch (msg.what) {
            ...
            case 21:
                ...
                int qrSize = SpUtil.readInt(QR_SIZE);
                String url = readString(Const.LOGO_URL);
    ...
    private void assignLoop() {
        ...
        for (Parameter parameter : startPlayFile) {
            ...
            if (fileIsExists(videoPaths)) {
                ...
                String enableVoice = SpUtil.readString(VOICE_ENABLE);
...
//                case 1:
//                    codec = activity.getIntent().getIntExtra("mediaCodec", AVOptions.MEDIA_CODEC_SW_DECODE);
//                    break;
//                case 2:
//                    codec = activity.getIntent().getIntExtra("mediaCodec", AVOptions.MEDIA_CODEC_HW_DECODE);
//                    break;
//                default:
//                    break;
//            }
            ...
            try {
                ...
                if (showSumSize > 0 && isJson) {
                    ...
                    if (isNetConnected(activity)) {
                        ...
                        String peakData = readString(Const.VIP_SHOW_DATA);
                        e("没有网络：" + peakData);
...
//                                    showUpdate(deviceId, playShowId, rCode, false);
                ...
                for (Parameter parameters : parameter) {
                    ...
                    if (adType == CONSTANT_ONE) {
                        ...
                        for (PlayList jiTouAdvs : jiTouAdv) {
                            ...
                            if (isDelete > 0) {
                                updateShowList();
                                String server = readString(Const.CONTROL_TYPE);
    ...
    private void appLogoInitialize() {
        ThreadManager.getInstance().doExecute(new Runnable() {
            ...
            public void run() {
                try {
                    String url = readString(Const.LOGO_SHOW_DATA);
                    e("logo路径：" + url);
                    logoShow(url);
                } catch (Exception e) {
                    e.printStackTrace();
